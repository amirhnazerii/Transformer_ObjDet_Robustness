...
#     parser = argparse.ArgumentParser('Set transformer detector', add_help=False)
    ...
    parser.add_argument('--cw_c', default=0.8, type=float)
    parser.add_argument('--cw_kappa', default=0, type=float)
    parser.add_argument('--cw_iters', default=200, type=int)
    parser.add_argument('--cw_lr', default=0.02, type=float)

    parser.add_argument('--dataset', default='', type=str, help='either coco or kitti')     ## Amir
    parser.add_argument('--model_name', default='', type=str, help='e.g. DETRr50dc5')     ## Amir
...
def new_attack(img_tensors, learning_rate, model, annotation, criterion, UnNorm, c, kappa, iters):        
        ...
        def f(x) :
            ...
            for ii, aux_outputs in enumerate(outputs['aux_outputs']):
                indices_aux = criterion.matcher(aux_outputs, [annotation[0]])
                src_logits_aux = aux_outputs['pred_logits']
                idx_aux = criterion._get_src_permutation_idx(indices_aux)
                target_classes_o_aux = torch.cat([t["labels"][J] for t, (_, J) in zip([annotation[0]], indices_aux)])
                target_classes_o_aux = target_classes_o_aux.to(device)
                target_classes_aux = torch.full(src_logits_aux.shape[:2], num_classes,
                                            dtype=torch.int64, device=src_logits_aux.device)
                target_classes_aux[idx_aux] = target_classes_o_aux
                
                one_hot_labels_aux = F.one_hot(target_classes_aux.to(src_logits_aux.device))
        
                i_temp, _ = torch.max((1-one_hot_labels_aux)*src_logits_aux, dim=2)
                i_aux += i_temp
                j_aux += torch.masked_select(src_logits_aux, one_hot_labels.bool())

            return torch.clamp(j-i+j_aux-i_aux, min=-kappa)
        ...
        out_init = model(img_tensors)            
        loss_dict = criterion(out_init, [annotation[0]])
        loss_init = -loss_dict['loss_ce'] - sum(loss_dict['loss_ce' + f'_{i}'] for i in range(args.dec_layers - 1))
        loss_init.backward()
        img_grad = img_tensors.tensors.grad
        img_init = img_tensors.tensors + 0.3*img_grad
        img_tensors.tensors = img_init.detach_()
        img_tensors.tensors.requires_grad = True
        ...
        for step in range(iters) :
            ...
            out = model(adv_tensors)            
            total_loss = criterion(out, [annotation[0]])

            loss3 = total_loss['loss_bbox'] + sum(total_loss['loss_bbox' + f'_{i}'] for i in range(args.dec_layers - 1))
            loss4 = total_loss['loss_giou'] + sum(total_loss['loss_giou' + f'_{i}'] for i in range(args.dec_layers - 1))
...
#                     # Call Our Attack:     
#                     if args.attack_type == 'new':
#                         perturbed_img = new_attack(img_tensors, args.cw_lr, model, annotation, criterion, UnNorm, args.cw_c, args.cw_kappa, args.cw_iters) # size: [1, 3, 800, 1201]
...
# #         print('img_tensors_list', img_tensors_list)                   
...
#         if save_img_tensors_list:
#             if args.dataset == 'coco':
#                 torch.save(img_tensors_list, "/scratch/anazeri/new_attack/coco/img_tensors_"+args.model_name+"_imgs"+str(i-3000)+".pt")            
#             if args.dataset == 'kitti':
#                 torch.save(img_tensors_list, "/scratch/anazeri/new_attack/coco/img_tensors_"+args.model_name+"_imgs"+str(i)+".pt")       
#         img_tensors_loaded= torch.load("/scratch/anazeri/new_attack/coco/img_tensors_1.pt")
        
        
        
        img_tensors_list1 = torch.load('/scratch/anazeri/new_attack/coco/img_tensors_DETRr101dc5_coco_imgs3000.pt')
        img_tensors_list2 = torch.load('/scratch/anazeri/new_attack/coco/img_tensors_DETRr101dc5_imgs1999.pt')
        img_tensors_list = img_tensors_list1 + img_tensors_list2
